...
for na=1:Nalfs,
    ...
    train_errorArr(na)=mean((tr_t-yest).^2)/global_var;
    pred_errorArr(na)=mean((tr_t-ypred).^2)/global_var;

alpha_opt_idx=find(pred_errorArr==min(pred_errorArr));
...
dyn_test_errorArr(na)=mean((zpred-te_t').^2)/global_var;
end
...
grid
figure(3)
plot(alf_array,dyn_test_errorArr,'b-',alf_array,pred_errorArr,'g-')
legend('Test error','Prediction error')
title('Relation between alpha and errors')
xlabel('\alpha')
ylabel('Error')