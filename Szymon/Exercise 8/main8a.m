...
plot_train_err = [];
for newk=1:200
    clearvars -except newk plot_test_err plot_train_err seed
    nits=30;         % Number of EM iterations
    method=1;        % Method of initialization 1,2,3
    common_sigs=1;   % =1 if clusters have common variances , =0 if not
    plot_motion=1;    % plot position of center while learning (time!)
    ...
    [y,sig2,prob_k]=gm_init(ztrain,K,method,seed);
    ...
    plot_test_err = [plot_test_err test_error];
    plot_train_err = [plot_train_err train_error];
...
% figure
figure
plot(diff(plot_test_err), 'r')
hold on
plot(diff(plot_train_err), 'b')
title('Square error')
legend('Test error', 'Train error')
ylabel('Square error')
xlabel('Value of K - numbe rof clusters')

%       Plot centers and widths
figure, plot(ztrain(:,D-1),ztrain(:,D),'.'),hold on,
for k=1:K,
