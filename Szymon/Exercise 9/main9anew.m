...
widthx=7;       % std of simulation normal distribution
widthy=15;       % std of simulation normal distribution
%
h_opt_avr = [];
for dataset_size = 1:500
    N=dataset_size;          % training set size
    h_opt_vec = [];

    for iter=1:10
        % Normal distributed data for training set
        xtrain=(Nx/2)+ widthx*randn(N,1);
        ytrain=(Ny/2)+ widthy*randn(N,1);
        ...
        % Normal distributed data for training set
        xval=(Nx/2)+ widthx*randn(Nval,1);
        yval=(Ny/2)+ widthy*randn(Nval,1);
        Zval=[xval,yval];
    ...
    %         disp(['Evaluating h ',int2str(n),' of ',int2str(Nh)]);
        ...
        h_opt_vec = [h_opt_vec h_opt];
    end
%     plot(h_opt_vec)
    disp(N)
    average  = mean(h_opt_vec);
    h_opt_avr = [h_opt_avr average];
end

%%
plot(h_opt_avr)
title('Dependency of h on dataset size')
ylabel('Optimal h')
xlabel('Dataset size')
