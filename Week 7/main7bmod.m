...
cNumbers=1:7;
nIt=1:3;
test_errors=zeros(size(cNumbers,2),size(nIt,2));
train_errors=zeros(size(cNumbers,2),size(nIt,2));
...
test_errors(K,i)= Etest_arr(nits);
train_errors(K,i)= Etrain_arr(nits);
    end
end
...
plot(cNumbers,nanmean(test_errors'))
hold on
plot(cNumbers, nanmean(train_errors'))
