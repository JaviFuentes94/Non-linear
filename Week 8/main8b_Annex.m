...
%           drawnow
  ...
  errorFinalTrain=Etrain_arr(nits);
  errorFinalTest=Etest_arr(nits);
...
testErrorVector(i,K-KNumbers(1)+1)=errorFinalTest;
trainErrorVector(i,K-KNumbers(1)+1)=errorFinalTrain;
testClassificationError(i,K-KNumbers(1)+1) = Etest;
end
end
...
figure(5)
subplot 211
plot(KNumbers,mean(testErrorVector));
hold on
plot(KNumbers,mean(trainErrorVector));
legend('Test error', 'Train Error')
title('Density estimation Error')

subplot 212
plot(KNumbers,mean(testClassificationError));
legend('Test error')
