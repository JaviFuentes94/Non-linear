...
testErrorVector=zeros(nIterations,length(KNumbers));
trainErrorVector=zeros(nIterations,length(KNumbers));
...
[y,sig2,prob_k]=gm_init(ztrain,K,method,seed+1);
...
for t=1:nits,
   ...
   sig2(sig2<sig2_thres)=sig2_thres;
...
testErrorVector(i,K-KNumbers(1)+1)=test_error;
trainErrorVector(i,K-KNumbers(1)+1)=train_error;
end
end
